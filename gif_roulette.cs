	...
	using (var webClient = new WebClient())
            {
                ...
                var jsonStr = webClient.DownloadString($"https://api.giphy.com/v1/gifs/random?api_key=YOUR_API_KEY&tag={searchTerm}&rating=pg13");

                // Parse the JSON response.
                var json = JObject.Parse(jsonStr);

                // Extract the URL property.
                var url = (string)json["data"]["embed_url"];
